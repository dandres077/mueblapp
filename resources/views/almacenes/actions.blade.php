<div class="dropdown dropdown-inline">
	...
	<div class="dropdown-menu dropdown-menu-right">
        @can('almacenes.edit')
		<a class="dropdown-item" href="{{ url('admin/almacenes/'.$id.'/edit')}}"><i class="la la-edit"></i>Editar</a>
		@endcan

		@can('almacenes.destroy')
		<form method="post" action="{{ url('admin/almacenes/'.$id)}}">
            @method('DELETE')
            {{ csrf_field() }}
        ...
        @if ($status==37)
            @can('almacenes.inactive')
            <form method="post" action="{{ url('admin/almacenes/'.$id.'/inactive')}}">
                {{ csrf_field() }}
                <button type="submit" type="button" class="dropdown-item"><i class="la la-info-circle"></i>&nbsp;&nbsp;&nbsp;Inactivar</button>
            </form>
            @endcan
        @else
            @can('almacenes.active')
            <form method="post" action="{{ url('admin/almacenes/'.$id.'/active')}}">
                {{ csrf_field() }}
