<div class="dropdown dropdown-inline">
	...
	<div class="dropdown-menu dropdown-menu-right">
        @can('precios.edit')
		<a class="dropdown-item" href="{{ url('admin/precios/'.$id.'/edit')}}"><i class="la la-edit"></i>Editar</a>
		@endcan

		@can('precios.destroy')
		<form method="post" action="{{ url('admin/precios/'.$id)}}">
            @method('DELETE')
            {{ csrf_field() }}
        ...
        @if ($status==122)
            @can('precios.inactive')
            <form method="post" action="{{ url('admin/precios/'.$id.'/inactive')}}">
                {{ csrf_field() }}
        ...
        @else
            @can('precios.active')
            <form method="post" action="{{ url('admin/precios/'.$id.'/active')}}">
                {{ csrf_field() }}
