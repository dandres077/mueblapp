...
class CreateCotizacionesDetsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('cotizaciones_dets', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('empresa_id')->index()->nullable();
            $table->unsignedBigInteger('producto_id')->index()->nullable();
            $table->unsignedBigInteger('cotizacion_id')->index()->nullable();
    ...
    {
        Schema::dropIfExists('cotizaciones_dets');
    }
}
